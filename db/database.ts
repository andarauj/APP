...
import { migrateExerciseMovementType, migratePurgeDuplicatePlanItems } from './planDedup';
...
export async function initDatabase(): Promise<void> {
  ...
  try {
    ...
    CREATE TABLE IF NOT EXISTS workout_sessions (
      ...
      notes TEXT DEFAULT '',
      adaptive_week_id INTEGER DEFAULT NULL,
  ...
  await runStep('migratePlanExerciseTargetRir', () => migratePlanExerciseTargetRir(db), db, applied);
  ...
  await runStep('migratePlanDaysJson', () => migratePlanDaysJson(db), db, applied);
  ...
  await runStep('migrateWorkoutSessionOrigin', () => migrateWorkoutSessionOrigin(db), db, applied);
  ...
  // Phase 5: EN/PT catalogue merges (Cable Crossover → Crossover no Cabo, …).
  await runStep('migrateExerciseCatalogNormalizeV4', () => migrateExerciseCatalogNormalize(db), db, applied);
  await runStep('migrateExerciseMovementType', () => migrateExerciseMovementType(db), db, applied);
  await runStep('migratePurgeDuplicatePlanItems', () => migratePurgeDuplicatePlanItems(db), db, applied);
}
 ...
 * - plan_exercises.target_rir is added in migratePlanExerciseTargetRir.
 */
...
/** Prescribed RIR on generated plan rows. Logged effort stays RPE. */
async function migratePlanExerciseTargetRir(db: SQLite.SQLiteDatabase): Promise<void> {
  const peCols = (await db.getAllAsync<{ name: string }>(`PRAGMA table_info(plan_exercises)`)).map(c => c.name);
  if (!peCols.includes('target_rir')) {
    await db.execAsync(`ALTER TABLE plan_exercises ADD COLUMN target_rir INTEGER DEFAULT NULL`);
  }
}
...
async function migratePlanDaysJson(db: SQLite.SQLiteDatabase): Promise<void> {
  const cols = (await db.getAllAsync<{ name: string }>(`PRAGMA table_info(workout_plans)`)).map(c => c.name);
  if (!cols.includes('days_json')) {
    await db.execAsync(`ALTER TABLE workout_plans ADD COLUMN days_json TEXT DEFAULT NULL`);
  }
}
...
/** Planned-workout origin: which adaptive week / phase a session was started from. */
async function migrateWorkoutSessionOrigin(db: SQLite.SQLiteDatabase): Promise<void> {
  const cols = (await db.getAllAsync<{ name: string }>(`PRAGMA table_info(workout_sessions)`)).map(c => c.name);
  if (!cols.includes('adaptive_week_id')) {
    await db.execAsync(`ALTER TABLE workout_sessions ADD COLUMN adaptive_week_id INTEGER DEFAULT NULL`);
  }
  if (!cols.includes('phase')) {
    await db.execAsync(`ALTER TABLE workout_sessions ADD COLUMN phase TEXT DEFAULT NULL`);
  }
}
...
/**
 * Runtime guard for generators: if the built-in catalogue was wiped or never
 * seeded, re-insert EXERCISE_SEED_DATA. Idempotent via INSERT OR IGNORE when
 * rows already exist.
 */
